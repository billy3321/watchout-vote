class Api::BillsController < ApplicationController
  ...
  def index
    ...
    if ransack_params.blank?
      @bills = Bill.date_desc.includes(:candidate, :issue, :committee).offset(params[:offset]).limit(params[:limit])
      @bills_count = Bill.count
    else
      @bills = Bill.date_desc.includes(:candidate, :issue, :committee).ransack(ransack_params).result(distinct: true)
        .offset(params[:offset]).limit(params[:limit])
      @bills_count = Bill.ransack(ransack_params).result(distinct: true).count
    end
    respond_with(@bills, @bills_count)
  end
